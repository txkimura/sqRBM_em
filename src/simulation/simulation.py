...
def get_pauli_kron_hidden(n_hidden):
    # set Kronecker product Pauli matrices
    pauli_kron_hidden = {}
    for i in range(0, n_hidden):
        pauli_kron_hidden["x", i] = sparse_kron(i, n_hidden, sparse_X)
        pauli_kron_hidden["z_diag", i] = sparse_kron(i, n_hidden, sparse_Z).diagonal()

    return pauli_kron_hidden
...
def compute_rho(H, beta, diagonal=False):
    ...
    if diagonal:
        ...
        # exp_beta_Lambda = np.exp(-beta * Lambda)
        # return (S * (exp_beta_Lambda / exp_beta_Lambda.sum())) @ S.T
